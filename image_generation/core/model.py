...
class MMD_GAN(object):
    def __init__(self, sess, config):
        ...
        self.description = ("%s%s_%s%s_%sd%d-%d-%d_%s_%s_%s_%.3g_%.3g_%.3g_%.3g_%i_%i_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%.3g_%i" % (
                            self.dataset, arch,
                            ...
                            int(self.config.with_scaling), int(self.config.with_sn), self.config.scaling_coeff, self.config.reg_ratio,
                            self.config.eps, self.config.gamma_1, self.config.gamma_2, self.config.m_ratio, self.config.lam_1, self.config.lam_2, self.config.lam_3, self.config.lam_4, self.config.pico, self.config.msteps))
        ...
        with tf.variable_scope('loss'):
            if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                ...
                if not self.config.rep:
                    ...
                    self.d_loss = -mmd.mmd2(kerGI, rep=0.5)
                self.optim_name = 'kernel_loss'
    ...
    def set_optimizer(self):
        with tf.device(self.consolidation_device):
            with tf.control_dependencies(self.update_ops):
                ...
                if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                    self.k_optim = tf.train.AdamOptimizer(self.k_ratio*self.lr * self.config.learning_rate_D / self.config.learning_rate, beta1=self.config.beta1, beta2=self.config.beta2)
                self.apply_grads()
...
#                 self.writer.add_summary(summary_str, step)
        ...
        if write_summary:
            if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                self.timer(step, "%s, G: %.8f, D: %.8f, Noise Norm: %.8f" % (self.optim_name, g_loss, d_loss, noise_norm))
            else:
        ...
        if self.config.is_demo:
            ...
            if self.d_counter == 0:  # run the generator update
                if write_summary:
                    ...
                    if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                        _, g_grads, d_grads, g_loss, d_loss, noise_norm_0 = self.sess.run([self.g_grads] + eval_ops + [self.actual_noise_norm])
                    else:
                        _, g_grads, d_grads, g_loss, d_loss = self.sess.run([self.g_grads] + eval_ops)
            else:

                if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                    _, _, g_grads, d_grads, g_loss, d_loss, noise_norm_0 = self.sess.run([self.d_grads, self.k_grads] + eval_ops + [self.actual_noise_norm])
        ...
        if self.d_counter == 0:
            if write_summary:
                if self.config.kernel == 'imp_1' or self.config.kernel == 'imp_2' or self.config.kernel == 'imp_3':
                    ...
                    np.savez(os.path.join(self.sample_dir, 'noise_norm_list.npz'), norm=np.asarray(self.noise_norm_list))
                else:
