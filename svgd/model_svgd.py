...
class Layer():
    def __init__(self, n_p, n_in, n_out, activation_fn=tf.nn.relu, imp=False, name='l1'):
        ...
        with tf.variable_scope(name) as scope:
            #### code  for implicit Bayesian Neural Network
            if imp:
                if self.n_in*self.n_out <= 50:
                    init_0 = tf.random_normal(shape=[self.n_p, 1])  # every time sample new particles
                    # init_0 = tf.random_uniform([self.n_p, 1], -1., 1.)  # every time sample new particles

                    # samples_, _ = any_function('', init_0, dim_f=self.n_in*self.n_out, layer_shape=[self.n_in*self.n_out//4. self.n_in*self.n_out//2], sn=False)    # this will provide more flexibility

                    # samples_, _ = any_function('', init_0, dim_f=self.n_in*self.n_out*2, layer_shape=[self.n_in*self.n_out//4], sn=False)
                    # samples_ = samples_[:,:self.n_in*self.n_out] + samples_[:,self.n_in*self.n_out:]*tf.random_normal(shape=[self.n_p, self.n_in*self.n_out])  # this will provide stable training

                    # self.w = tf.reshape(samples_, shape=[self.n_p, self.n_in, self.n_out])
                    self.w = tf.get_variable('w1', shape=(self.n_p, self.n_in, self.n_out), dtype=tf.float32, initializer=tf.glorot_uniform_initializer()) \
                             + tf.get_variable('w2', shape=(self.n_p, self.n_in, self.n_out), dtype=tf.float32, initializer=tf.glorot_uniform_initializer())*tf.random_normal(shape=[self.n_p, 1, 1])

                    self.params = [self.w]
                else:
                    w0 = (1.0 / np.sqrt(self.n_in + 1) * np.random.randn(self.n_p, self.n_in, self.n_out)).astype(
                        'float32')
    ...
    def __init__(self, config):
        ...
        for i in range(len(self.n_neurons) - 1):
            ...
            if i == len(self.n_neurons) - 2:
                activation_fn = None
            self.nnet.append(Layer(self.config.n_particles, self.n_neurons[i], self.n_neurons[i+1], activation_fn=activation_fn, imp=self.config.imp, name='l_%d' % i) )
        ...
        # + 10 * tf.reduce_mean(tf.square(self.y - self.y_pred))
        #############################################################
        ...
        # self.A = [tf.Variable(tf.zeros((self.config.n_particles, p.n_in, p.n_out)), trainable=False, dtype=tf.float32) for p in self.nnet]
        # self.G = [tf.Variable(tf.zeros((self.config.n_particles, p.n_in, p.n_out)), trainable=False, dtype=tf.float32) for p in self.nnet]
        ...
        for p, g in zip(self.train_vars, self.net_grads):
            ...
            # if self.config.kernel == 'hk' or self.config.kernel == 'hkdk':
            #    svgd_grad = tf.clip_by_value(svgd_grad, -10., 10.)
        ...
        if self.config.kernel == 'hk':
            ...
            kernel_matrix, _ = hk_kernel(p)
            self.mar = marginal(kernel_matrix, self.config.n_particles)
            sinkhorn_loss, _ = compute_sinkhorn_loss(p, kernel_matrix, self.config.n_particles, self.mar.mu, self.mar.nu)
            energy_loss = tf.reduce_sum(delete_diag(tf.log(kernel_matrix + 1e-7)) * self.mar.mu) / (self.config.n_particles - 1)
            self.kernel_loss += -self.config.gamma_3 * tf.reduce_sum(delete_diag(kernel_matrix)) / (self.config.n_particles * (self.config.n_particles - 1)) + self.config.gamma_1 * sinkhorn_loss + self.config.gamma_2 * energy_loss
